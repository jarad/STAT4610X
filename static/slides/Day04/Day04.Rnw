...
\subsection{R code}
\begin{frame}[fragile]
\frametitle{Poisson process per game}
...
These intervals are interpreted \alert{per game} played. 
\end{frame}
...
\includegraphics{driving_distance}
\end{frame}
...
\pause
\[ 
\overline{y} \pm z_{a/2} \times s / \sqrt{n}
\]
\pause 
where 
\begin{itemize}
\item $\overline{y} = \frac{1}{n}\sum_{i=1}^n y_i$ is the sample mean and
\item $s = \frac{1}{n-1} \sum_{i=1}^n (y_i - \overline{y})^2$ is the sample variance.
\end{itemize}
...
This is the uncertainty around his mean driving distance.
\end{frame}
...
In this slide set, we introduced some basic uncertainty intervals for using
